	...
	)
{
	...
	{
		CString sR,sG,sB;
		sR.Format("%d", GetRValue(clr) );
		sG.Format("%d", GetGValue(clr) );
		sB.Format("%d", GetBValue(clr) );
	...
	if( bSyncCMY )
	{
		...
		CString sC,sM,sY;
		sC.Format("%d", GetRValue(clrCMY) );
		sM.Format("%d", GetGValue(clrCMY) );
		sY.Format("%d", GetBValue(clrCMY) );

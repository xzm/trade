	...
	)
{
	...
	{ // if needs re-create anything
		...
		CBitmap * pOldBitmap = m_dcCube.SelectObject( &bmp );
		...
		m_dcCube.SelectObject( pOldBitmap );
		if( pOldPalette != NULL )
	...
	)
{
	...
	{ // if needs re-create anything
		...
		// MODIFIED BY SUNZHENYU, add the next 1 line
		m_dcCube.SelectObject( pOldBitmap );
		if( pOldPalette != NULL )
			m_dcCube.SelectPalette( pOldPalette, FALSE );
	...
	)
{
	...
	if( m_dcPicker.GetSafeHdc() == NULL )
	{
		...
		// MODIFIED BY SUNZHENYU, modified the next 1 line
		CBitmap * pOldBitmap = m_dcPicker.SelectObject( &bmp );
		...
		nBorder -= m_nAreaGap*2;
		double fBorder = nBorder / 2;
		int cx = m_sizeColorPicker.cx / 2; // nBorder / 2 + 1;
		int cy = m_sizeColorPicker.cy / 2; //cx;
		...
		for( int i = 360; i--; )
		{
			...
			arrPt[i].x =
				int( cx - ::cos(PI*i/180.0)*fBorder );
			arrPt[i].y =
				int( cy - ::sin(PI*i/180.0)*fBorder );
		}
		...
		for( i = 360; i--; )
		{
			...
			m_dcPicker.MoveTo(arrPt[i]);
			m_dcPicker.LineTo( cx, cy );
			m_dcPicker.MoveTo( arrPt[i].x+1, arrPt[i].y );
			m_dcPicker.LineTo( cx, cy );
			m_dcPicker.MoveTo( arrPt[i].x+2, arrPt[i].y+1 );
			m_dcPicker.LineTo( cx, cy );
			m_dcPicker.MoveTo( arrPt[i].x-1, arrPt[i].y );
			m_dcPicker.LineTo( cx, cy );
			m_dcPicker.MoveTo( arrPt[i].x-1, arrPt[i].y+1 );
			m_dcPicker.LineTo( cx, cy );
			m_dcPicker.MoveTo( arrPt[i].x-1, arrPt[i].y-1 );
			m_dcPicker.LineTo( cx, cy );
			::DeleteObject(
...
*/
		...
		CRect rcEllipse( cx, cy, cx, cy );
		int nRad = ((nBorder*2)/3)/2;
		rcEllipse.InflateRect( nRad, nRad );
		::SelectObject(
			m_dcPicker.GetSafeHdc(),
		...
		HGDIOBJ oldBrush =
			::SelectObject(
				m_dcPicker.GetSafeHdc(),
				::CreateSolidBrush( DEF_HSL_TRANSPARENT )
				);
		m_dcPicker.Ellipse( &rcEllipse );
		::DeleteObject(
			::SelectObject(
				m_dcPicker.GetSafeHdc(),
				oldBrush
		...
		// MODIFIED BY SUNZHENYU, add the next 1 line
		m_dcPicker.SelectObject( pOldBitmap );
...
int cy = m_sizeColorPicker.cy / 2; //cx;
int iHue =
		int( m_fCurrHue * (-360.0) );
CPoint ptLum(
		int( cx - ::cos(PI*iHue/180.0)*fBorder ),
		int( cy - ::sin(PI*iHue/180.0)*fBorder )
	 );

SIZE sz = { 6, 6 };
	DrawCrossAt(
		ptLum + CPoint(m_nAreaGap,m_nAreaGap),
		dc,
		sz
...
CPoint pt(	(int)(m_fCurrLuminance*m_sizeColorSlider.cx),
	...
	DrawCrossAt(
		pt + CPoint(m_nAreaGap,m_nAreaGap*2),
		dc,
		sz
		); //draw rectangle on luminance bar
	...
	if( m_bBmpValidPicker && (nFlags & MK_LBUTTON) )
	{
		...
		else if(m_eMoveMode == _MM_PICKER)
		{
			...
			if( point.x >= m_sizeColorPicker.cx )
				point.x = m_sizeColorPicker.cx - 1;
			if( point.y > m_sizeColorPicker.cy )
				point.y = m_sizeColorPicker.cy;

			m_ptMousePos = point;
			COLORREF clr = m_dcPicker.GetPixel( m_ptMousePos );
			if( clr != DEF_HSL_TRANSPARENT
				&& m_clr != clr
				)
			{
				//OnSetRGB( clr );
				double fHue,fSaturation,fLuminance;
				CExtPaintManager::stat_RGBtoHSL(
					clr,
					&fHue,
					&fSaturation,
				...
				fLuminance = m_fCurrLuminance;
				fSaturation = m_fCurrSaturation;
				OnSetHLS(
					fHue,
					fLuminance,
					fSaturation
	...
	)
{
	...
	if( m_dcSlider.GetSafeHdc() == NULL )
	{
		...
		// MODIFIED BY SUNZHENYU, modified the next 1 line
		CBitmap * pOldBitmap = m_dcSlider.SelectObject( &bmp );
		...
		// MODIFIED BY SUNZHENYU, add the next 1 line
		m_dcSlider.SelectObject( pOldBitmap );
		if( pOldPalette != NULL )
...
SIZE sz = { 6, 6 };
	DrawCrossAt(
		m_ptMousePos + CPoint(m_nAreaGap,m_nAreaGap),
		dc,
		sz
...
SIZE sz = { 6, 6 };
	DrawCrossAt(
		CPoint(nX,nY) + CPoint(m_nAreaGap,m_nAreaGap),
		dc,
		sz
		); // draw a rectangle on picker
...
//	VERIFY(
//		CExtPaintManager::stat_PaintParentBk(
//			GetSafeHwnd(),
//			dc,
//			&rcClient
//			)

	...
	)
{
   for(int x=0; x<64; x++)
   {
      mapPalColor(x,
         ( (int) ( ((float)x) * 4 * redFx   ) ),
         ( (int) ( ((float)x) * 4 * greenFx ) ),
         ( (int) ( ((float)x) * 4 * blueFx  ) ) );
      mapPalColor(127-x,
         ( (int) ( ((float)x) * 4 * redFx   ) ),
         ( (int) ( ((float)x) * 4 * greenFx ) ),
         ( (int) ( ((float)x) * 4 * blueFx  ) ) );
...
int old_bk_mode = mdc.SetBkMode(TRANSPARENT);
	...
	// draw things
COLORREF clr3DText = GetSysColor(COLOR_BTNTEXT);
COLORREF clr3DFace = GetSysColor(COLOR_3DFACE);
COLORREF clr3DLight = GetSysColor(COLOR_3DHIGHLIGHT);
...
//		mdc.DrawIcon(crIcon.TopLeft(),hIcon);
	...
	if(m_bEnableResizing)
	{
		...
		CFont * pOldFontLocal = mdc.SelectObject(&font);
		...
		// MODIFIED BY SUNZHENYU, modified next 1 line
		mdc.SelectObject( pOldFontLocal );
	} // if(m_bEnableResizing)


	// finish this things!
	// MODIFIED BY SUNZHENYU, remove next 1 line
	// if(pOldFont)

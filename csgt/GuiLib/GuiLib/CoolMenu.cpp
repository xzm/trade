...
BOOL CCoolMenuManager::OnMeasureItem(LPMEASUREITEMSTRUCT lpms)
{
	...
	if (pmd->fType & MFT_SEPARATOR) {
		...
		lpms->itemHeight= max(GetSystemMetrics(SM_CYMENU), rcText.Height())+4;
...
void CCoolMenuManager::DrawSeparator(CRect& rc, CDC& dc)
{
	...
	m_rc.right=m_rc.left+m_szButton.cx;
	dc.FillRect(m_rc,&brush);
	brush.DeleteObject( );
	brush.CreateSolidBrush(RGB(255,255,255));
	rc.left+=m_szButton.cx;
...
void CCoolMenuManager::FixMFCDotBitmap()
{
	...
	if (hbmDot) {
		...
		HBITMAP hbmOld = (HBITMAP)memdc.SelectObject((HGDIOBJ)hbmDot);

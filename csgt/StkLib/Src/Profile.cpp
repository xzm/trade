...
#ifdef CLKVER_OEM
	strResult	=	OEM_SERVER_DOMAIN;
	if( 0 == strResult.Find( "www." ) )
		strResult	=	strResult.Mid( 4 );
#else
...
#else
	strResult.Format( "http://%s:%d/download/clking/upgrade3.exe", (LPCTSTR)GetHTTPServerDomain(), GetHTTPServerPort() );
#endif
	return strResult;
}
...
{
#ifdef	CLKLAN_ENGLISH_US
	return GetWorkDirectory() + "data\\domainEnu.dat";
#else
	return GetWorkDirectory() + "data\\domain.dat";
#endif
}
...
{
#ifdef	CLKLAN_ENGLISH_US
	return GetWorkDirectory() + "groupEnu.dat";
#else

...
#if	!defined( __STKLIB_SELFDB_H__ )
#define	__STKLIB_SELFDB_H__
...
/*	适用各种数据类型存储的文件格式包含三部分
	...
	2. 索引区：大小为CLK_FHEADER.m_dwIndexRecordSize * CLK_FHADER.m_dwIndexRecordCount
				分为CLK_FHADER.m_dwIndexRecordCount个块，
				每块为CLK_FHEADER.m_dwIndexRecordSize，即CLK_INDEXRECORD
	
	3. 数据区：大小为CLK_FHEADER.m_dwBlockSize * CLK_FHEADER.m_dwBlockCount
				分为CLK_FHADER.m_dwBlockCount个Block，
				每一Block有一个 CLK_BLOCKHEADER，和紧随其后的CLK_BLOCKHEADER.m_dwDataRecordCount个股票数据记录
				如果一个Block不能记录该股票全部记录，则CLK_BLOCKHEAHDRE.m_dwPosNextBlock指向下一个数据块
*/
...
typedef	struct clk_fheader	{	// 自有文件格式，文件头
	// 文件信息
...
typedef	struct clk_indexrecord	{		// 自有文件格式，股票索引部分单元
...
typedef	struct clk_blockheader	{		// 自有文件格式，数据区部分，数据块头
...
public:
	...
	// operation
	static	BOOL	BuildEmptyFile( LPCTSTR lpszFileName,
						DWORD dwDataType,	// 数据分类说明，see CStock::DataType
						DWORD dwMarket,			// 市场，see CStock::StockMarket，如果文件包含多个市场，则=0无效
						DWORD dwIndexRecordCount = 3000,	// 索引区记录单元个数
						DWORD dwRecordPerBlock = 50,		// 每个Block的记录数
						BOOL bRebuildIfExists = FALSE
						);
	static	BOOL	RemoveFileIfOutoftime( LPCTSTR lpszFileName, time_t tmNow );	// return TRUE if removed
	
	// Store Function
	BOOL	EmptyAll( );
	DWORD	StoreDataRecord(DWORD dwMarket, const char * szCode,

...
CClickLink::CClickLink( )
{
	m_nMode		=	modeNone;
	m_dwData	=	0;
}

void CClickLink::SetModeHelp( DWORD dwData, LPCTSTR lpszParam )
{
	m_nMode		=	modeHelp;
	m_dwData	=	dwData;
	if( lpszParam )
...
void CClickLink::SetModeRun( void (*runfunc)() )
{
	m_nMode		=	modeRun;
	m_runfunc	=	runfunc;
}

BEGIN_MESSAGE_MAP(CClickLink, CHyperLink)
	//{{AFX_MSG_MAP(CClickLink)
	ON_CONTROL_REFLECT(STN_CLICKED, OnClicked)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CClickLink message handlers

void CClickLink::OnClicked( )
{
	switch( m_nMode )
	{
	case modeHelp:
		::HtmlHelp(NULL, m_strParam, HH_DISPLAY_TOPIC, 0);
		break;
	case modeRun:
		if( m_runfunc )
			(*m_runfunc)();

...
CGridCtrl::CGridCtrl(int nRows, int nCols, int nFixedRows, int nFixedCols)
{
	...
	SetSelectedBkColor(::GetSysColor(COLOR_HIGHLIGHT));
	...
	m_SelectedCellMap.InitHashTable( 2048 );
	m_PrevSelectedCellMap.InitHashTable( 2048 );
}
    ...
    switch (nSBCode)
    {
        ...
        case SB_PAGEUP:
            if (scrollPos > 0)
            {
                rect.top = GetFixedRowHeight();
				int	nHeightSum = 0, nOffset = 0;
				while( nHeightSum < rect.Height() && idTopLeft.row+nOffset >= GetFixedRowCount() )
				{
					nHeightSum	+=	GetRowHeight(idTopLeft.row+nOffset);
					nOffset	--;
				}
                int pos = max(0, scrollPos + nOffset*VSCROLL_FACTOR);
...
CCellID CGridCtrl::GetTopleftNonFixedCell() const
{
	...
	int	nRow	=	max( GetFixedRowCount(), min( nVertScroll/VSCROLL_FACTOR, GetRowCount()-1 ) );
...
BOOL CGridCtrl::SetOuterHScrollBar( BOOL bOuterHScrollBar, HWND hwndOuterHScroll )
{
	if( bOuterHScrollBar )
	{
		FlatSB_ShowScrollBar( GetSafeHwnd(), SB_HORZ, FALSE );
		ModifyStyle( WS_HSCROLL | WS_DLGFRAME | WS_BORDER | WS_THICKFRAME, 0, SWP_DRAWFRAME );
	}
	else
	{
		FlatSB_ShowScrollBar( GetSafeHwnd(), SB_HORZ, TRUE );
		ModifyStyle( 0, WS_HSCROLL | WS_DLGFRAME | WS_BORDER | WS_THICKFRAME, SWP_DRAWFRAME );
	}

	m_bOuterHScrollBar = bOuterHScrollBar;
	m_hwndOuterHScroll = hwndOuterHScroll;
	return m_bOuterHScrollBar;
}
...
void CGridCtrl::ResetScrollBars()
{
	...
	si.fMask  = SIF_PAGE;
	si.nPage  = VisibleRect.Width();    FlatSB_SetScrollInfo(GetSafeHwnd(),SB_HORZ, &si, FALSE); 
	si.nPage  = VisibleRect.Height();   FlatSB_SetScrollInfo(GetSafeHwnd(),SB_VERT, &si, FALSE); 

	if (VisibleRect.Height() < VirtualRect.Height() )
		m_nVScrollMax = GetRowCount()*VSCROLL_FACTOR + VisibleRect.Height(); // VirtualRect.Height()-1; //+ GetRowHeight(visibleCells.GetTopLeft().row);
	else
		m_nVScrollMax = 0;
	...
	ASSERT(m_nVScrollMax < SHRT_MAX && m_nHScrollMax < SHRT_MAX);
	FlatSB_SetScrollRange(GetSafeHwnd(),SB_VERT, 0, m_nVScrollMax, TRUE);
	FlatSB_SetScrollRange(GetSafeHwnd(),SB_HORZ, 0, m_nHScrollMax, TRUE);

	if( m_bOuterHScrollBar )
	{
		FlatSB_ShowScrollBar( GetSafeHwnd(), SB_HORZ, FALSE );
		ModifyStyle( WS_HSCROLL | WS_DLGFRAME | WS_BORDER | WS_THICKFRAME, 0, SWP_DRAWFRAME );
	}
...
int CGridCtrl::GetScrollPos32(int nBar, BOOL bGetTrackPos /* = FALSE */) const
{
	...
	if( m_bOuterHScrollBar && SB_HORZ == nBar )
	{
		if( bGetTrackPos )
		{
			si.fMask	=	SIF_TRACKPOS;
			if( ::GetScrollInfo( GetSafeHwnd(), nBar, &si ) )
				return si.nTrackPos;
		}
		else 
		{
	...
	return ::GetScrollPos( GetSafeHwnd(), nBar );
}

int CGridCtrl::SetScrollPos32(int nBar, int nPos, BOOL bRedraw /* = TRUE */)
{
	SCROLLINFO si;
	si.cbSize = sizeof(SCROLLINFO);
	si.fMask  = SIF_POS;
	...
	if( m_bOuterHScrollBar && SB_HORZ == nBar )
	{
		int nRet = ::SetScrollPos( GetSafeHwnd(), nBar, nPos, bRedraw );
		::SendMessage( m_hwndOuterHScroll, WM_HSCROLL, MAKEWPARAM(SB_THUMBPOSITION,nPos), 0 );
		return nRet;
	}
	else
...
#define AUTOSIZE_COLUMN_MIN 20
#define AUTOSIZE_COLUMN_MAX 200
...
BOOL CGridCtrl::AutoSizeColumn(int nCol)
{
    ...
    int nWidth = AUTOSIZE_COLUMN_MIN;
    ...
    if( nWidth > AUTOSIZE_COLUMN_MAX )
        nWidth = AUTOSIZE_COLUMN_MAX;
...
void CGridCtrl::AutoSize()
{
    ...
    for (int nCol = 0; nCol < nNumColumns; nCol++)
        m_arColWidths[nCol] = AUTOSIZE_COLUMN_MIN;
    ...
    for (nCol = 0; nCol < nNumColumns; nCol++)
    {
        if( m_arColWidths[nCol] > AUTOSIZE_COLUMN_MAX )
            m_arColWidths[nCol] = AUTOSIZE_COLUMN_MIN;
    }
...
BOOL CGridCtrl::DrawCell(CDC* pDC, int nRow, int nCol, CRect rect, BOOL bEraseBk)
{
    ...
    {
        rect.right++; rect.bottom++;    // FillRect doesn't draw RHS or bottom
        pDC->FillSolidRect(rect, GetSelectedBkColor() );
        rect.right--; rect.bottom--;
